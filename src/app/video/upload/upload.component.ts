...
export class UploadComponent implements OnDestroy {
  ...
  async storeFile($event: Event) {
    ...
    const fileDropped = ($event as DragEvent).dataTransfer;
    const fileInputed = ($event.target as HTMLInputElement).files;

    this.file = fileDropped
      ? fileDropped?.files.item(0) ?? null
      : fileInputed?.item(0) ?? null;
